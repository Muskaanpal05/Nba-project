...
export default function ParticularGame(props) {
    const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
    const { data } = props;
    function handellingCloseFxn(e) {
        e.stopPropagation();
        props.close();
    }
    return (
        <Dialog open={props.open} onClose={handellingCloseFxn}>
            ...
            <DialogContent style={{margin: "0 auto", padding: "4rem"}} contentStyle={{width: "100%", maxWidth: "none"}}>
                ...
                <Button size="large" style={{ background: "#60cb5c", color: "white" }} onClick={handellingCloseFxn}>Close</Button>
                </Grid>
            </DialogContent>
        </Dialog>
    );
